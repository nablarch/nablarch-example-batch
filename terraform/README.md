...
- `ec2.tf`
    ...
    - 2つの EC2 インスタンス
        - 踏み台サーバー(`performance-batch-bastion-instance`)
        - 性能検証の対象となるバッチアプリケーションを稼働させる EC2 インスタンス(`performance-batch-ap-instance`)
- `rds.tf`
...
- アプリケーションのビルドと配備
- テストで使用する全国一括の郵便番号CSVファイルのダウンロードと配備
...
Terraform 実行のために以下のツールが必要になるので、あらかじめインストールしておくこと。
...
- EIP が作成済みであること
    - EC2 インスタンスに割り当てる EIP は事前に作成している前提
...
なお、踏み台サーバーの名前は `performance-batch-bastion-instance` で構築されている。
...
### APサーバーに接続できるか確認
```bash
# AP サーバー(performance-batch-ap-instance)
$ ssh ec2-user@172.17.2.10
```

- SSH の公開鍵は、 `performance-batch-bastion-instance` に配備したのものと同じ鍵が配備されている
- また、 `performance-batch-bastion-instance` の `.ssh/id_rsa` に秘密鍵を配備しているので、特に準備作業をせずに SSH 接続の確認ができる
- IP アドレスは、 `ec2.tf` ファイルを変更していなければ上記アドレスで接続できるように構築されている

## テストの実行

`performance_test/README.md` を参照。
